...
fn draw_sparkle(c: &mut GpuCanvas, s: &radial_common::RadialSetup, cx: f32, cy: f32, size: f32, col: Color, alpha: f32) {
    ...
    c.set_shadow(s.glow, (10.0 + s.bs * 8.0) * s.size_scale);
    c.fill_circle(cx, cy, (1.8 + size * 0.18) * s.size_scale);
}
...
pub fn render(c: &mut GpuCanvas, ctx: &mut RenderContext) {
    ...
    let num_stars = (ctx.config.reactivity.bar_count / 2).clamp(8, 64);
    ...
    for st in 0..num_stars {
        ...
        let star_r = s.inner_r + (40.0 + val * 80.0 + s.bs * 15.0) * s.size_scale;
        let size = (5.0 + val * 10.0) * s.size_scale;
        ...
        if st % 2 == 0 {
            ...
            let sx2 = s.cx + a2.cos() * (s.inner_r + 70.0 * s.size_scale);
            let sy2 = s.cy + a2.sin() * (s.inner_r + 70.0 * s.size_scale);
            draw_sparkle(c, &s, sx2, sy2, size * 0.6, s.accent, 0.35 + (1.0 - base_wave) * 0.35);
        }
    }
