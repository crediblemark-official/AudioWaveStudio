...
//! - Glowing 3D synthwave horizon sun disc & 3D highway grid floor.
//! - 24 rows of 3D equalizer buildings surging on both sides of the highway.
...
pub fn render(c: &mut GpuCanvas, ctx: &mut RenderContext) {
    ...
    scene.cam_pitch = -0.25 - be * 0.04;
    scene.cam_zoom = (0.92 - be * 0.06) / user_scale;
    ...
    scene.push();
    scene.translate(0.0, 40.0 * user_scale, -700.0 * user_scale);
    scene.add_disc(0.0, 0.0, 0.0, sun_r, 32, mix(accent_col, Color::hex("#ff007f"), 0.85));
    scene.pop();

    // B. 3D EQUALIZER BUILDINGS ALONG HIGHWAY SIDES
    for row in 0..BUILDING_ROWS {
        let row_f = row as f32;
        let z_pos = -row_f * (35.0 * user_scale) + (frame_time * 120.0 % (35.0 * user_scale));
        ...
        // Left Building
        let col_l = mix(p_col, glow_col, fv_l);
        scene.push();
        ...
        // Right Building
        let col_r = mix(s_col, accent_col, fv_r);
        scene.push();
